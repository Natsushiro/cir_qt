...
@torch.no_grad()
def circo_generate_test_submission_file(dataset_path: str, clip_model: str, ref_names_list: List[str],
    ...
    #submissions_folder_path = 'circo'
    #os.mkdir(submissions_folder_path)

    with open("circo_camt03.json", 'w+') as file:
...
def circo_generate_test_dict(relative_test_dataset: CIRCODataset, clip_model: CLIP, index_features: torch.Tensor,
    ...
    # Normalize the features
    index_features = index_features.float().cuda()
...
@torch.no_grad()
def cirr_generate_test_submission_file(dataset_path: str, clip_model: CLIP, ref_names_list: List[str],
    ...
    # Load the CLIP model
    #clip_model, _ = clip.load(clip_model_name, device="cuda", jit=False)
    ...
    with open("cirr_noise_icip.json", 'w+') as file:
        json.dump(submission, file, sort_keys=True)

    with open("cirr_noise_icip_group.json", 'w+') as file:
...
def cirr_generate_test_dicts(relative_test_dataset: CIRRDataset, clip_model: CLIP, index_features: torch.Tensor,
    ...
    index_features = index_features.to("cuda")
...
def main_worker(gpu, ngpus_per_node, log_queue, args):
    ...
    relative_val_dataset = CIRRDataset("../data/CIRR", 'test', 'relative', preprocess_val, no_duplicates=True)
    ...
    cirr_generate_test_submission_file("../data/CIRR", model, ref_names_list, pseudo_tokens,
                                            preprocess_val, "submit_circo")
