...
from webdata import get_wds_dataset
from pycocotools.coco import COCO
import nltk

def get_paths(path='../data/coco', use_restval=True):
    roots, ids = {}, {}
    imgdir = os.path.join(path, 'images')
    capdir = os.path.join(path, 'annotations')
    roots['train'] = {
        'img': os.path.join(imgdir, 'trainval35k'),
...
class CIRR(Dataset):
    ...
    vis_mode=False, test=False, root='./data'):
        ...
        self.test = test
        self.root = os.path.join(root, '../data/CIRR')
    ...
    def return_valdata(self, idx):
        if self.mode == 'caps' and not self.vis_mode:
            ...
            #text_with_blank = '* , {}'.format(target_cap)
            caption_only = tokenize(target_cap)[0]
...
class FashionIQ(Dataset):
    def __init__(self, cloth, transforms, is_train=False, vis_mode=False, \
        ...
        root_iq = os.path.join(root, '../data/fashion-iq')
    ...
    def return_all(self, idx):
        ...
        #text_with_blank = '* {} and {}'.format(cap2, cap1)
...
class CsvCOCO(Dataset):
    def __init__(self, transforms, transforms_region, sep=",",
        ...
        self.root_img = os.path.join(self.root, 'images/val2017')
...
def get_coco_dataset(args, preprocess_fn, is_train, epoch=0):
    roots, ids = get_paths()
    #roots, ids = get_paths_1000(use_restval=False)
    if is_train:
        split = 'train'
    else:
        split = 'test'
