...
# Most code is from https://github.com/openai/CLIP
import hashlib
import os
import urllib
import warnings
...
from model.model_context import build_model
...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit=True, is_train=False, pretrained=True):
    ...
    if name in _MODELS:
        ...
        model_id, filename = os.path.split(_OPENAI[name])
        if filename:
            model_path = download_pretrained_from_hf(model_id, filename=filename)
        else:
            model_path = download_pretrained_from_hf(model_id)
    else:
