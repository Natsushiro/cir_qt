...
class DifferentialNet(nn.Module):
    def __init__(self, embed_dim=768*2, middle_dim=512, output_dim=768, n_layer=2, dropout=0.1):
        super().__init__()
...
class MoCo(nn.Module):
    ...
    def forward(self, img, txt, clip_model):
        ...
        #img_k = img_features.clone().detach()

        # begin to retrieve for the similar samples
        retrieval_mat = torch.einsum("nc,kc->nk", [img_features, self.queue_txt.clone().detach()])
        ind_similar = retrieval_mat.argmax(dim=-1) # n*1. it's also the "labels"
        t_similar_features = self.queue_txt.clone().detach()[ind_similar] # n*c
        #print(ind_similar.shape)
        #print(txt_features.shape)
        #print(t_similar_features.shape)
        #i_similar_features = self.queue_img.clone().detach()[ind_similar] # n*c. This is noted as the gt image?

        diff_features = self.diff_net_q(txt_features, t_similar_features) # normalized, n*c
        #diff_features = t_similar_features - txt_features
