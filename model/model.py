...
class ResidualAttentionBlock(nn.Module):
    ...
    #def attention(self, x: torch.Tensor, attn_mask: torch.Tensor = None, attn_out: List[torch.Tensor] = None):
    #    if attn_mask is None:
    #        attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
    #    if attn_out is None:
    #        return self.attn(x, x, x, need_weights=False, attn_mask=attn_mask)[0]
    #    else:
    #        output, weight = self.attn(x, x, x, need_weights=True, attn_mask=attn_mask)
    #        attn_out.append(weight)
    #        return output

    #def forward(self, x: torch.Tensor, attn_mask: torch.Tensor = None, attn_out: List[torch.Tensor] = None):
    #    x = x + self.attention(self.ln_1(x), attn_mask=attn_mask, attn_out=attn_out)
    #    x = x + self.mlp(self.ln_2(x))
    #    return x
...
class Transformer(nn.Module):
    ...
    #def forward(self, x: torch.Tensor, attn_mask: torch.Tensor = None, attn_out: List[torch.Tensor] = None):
    #    return self.resblocks(x, attn_mask=attn_mask, attn_out=attn_out)
...
class VisualTransformer(nn.Module):
    ...
    #            attn_mask: torch.Tensor = None, attn_out: List[torch.Tensor] = None):
    #    # mask: MAE-like masking; reduce computation
    #    # attn_mask: apply on MultiheadAttention; 
    #    #            allow different mask rates for images for the same batch
    #    # attn_out: a list to save the attention maps
#
    #    x = self.conv1(x)  # shape = [*, width, grid, grid]
    #    x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
    #    x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
...
#
    #    x = x.permute(1, 0, 2)  # NLD -> LND
    #    if mask is not None:
    #        x = x.gather(0, mask.unsqueeze(-1).expand(-1, -1, x.size(2)))
    #    x = self.transformer(x, attn_mask=attn_mask, attn_out=attn_out)
    #    x = x.permute(1, 0, 2)  # LND -> NLD
#
    #    x = self.ln_post(x[:, 0, :])
#
...
class CLIP(nn.Module):
    ...
    def encode_text_mask(self, text, img_tokens, pos):
        b_size = img_tokens.size(0)
        x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
        collect_ind = text == self.end_id
        collect_ind = collect_ind.nonzero()[:, 1]
        x[range(b_size), pos] = img_tokens.type(self.dtype)
        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
