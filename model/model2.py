...
# New for 2words
class IM2TEXT_2words(nn.Module):
    def __init__(self, embed_dim=512, middle_dim=512, output_dim=512, n_layer=2, dropout=0.1):
        super().__init__()
        self.fc_out1 = nn.Linear(middle_dim, output_dim)
        self.fc_out2 = nn.Linear(middle_dim, output_dim)
        layers1 = []
        layers2 = []
        dim = embed_dim
        for _ in range(n_layer):
            ...
            dim = middle_dim
            layers1.append(nn.Sequential(*block))
        ...
        for _ in range(n_layer):
            ...
            block.append(nn.ReLU())
            dim = middle_dim
            layers2.append(nn.Sequential(*block))          
...
class IM2TEXT_transformer(nn.Module):
    def __init__(self, embed_dim, word_embed, length=8, num_layer=4, num_head=4):
        ...
        self.transformer = Transformer(word_embed, num_layer, num_head)
...
class CLIP(nn.Module):
    ...
    def encode_text_img(self, text, img_tokens):
        ...
        img_token1 = img_tokens[0].view(b_size, 1, -1)
        img_token2 = img_tokens[1].view(b_size, 1, -1)
        #x = torch.cat([x[:, :collect_ind[0]], img_tokens, x[:, 1 + collect_ind[0]:-1]], dim=1)
        x = torch.cat([x[:, :ind_insert1], img_token1,
                    x[:, 1 + ind_insert1:ind_insert2],
                    img_token2,
                    x[:, 1 + ind_insert2:]], dim=1)
        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        ...
        for i, sample in enumerate(x):
            ...
            if isinstance(img_tokens, tuple):
                ...
                for i, index in enumerate(indexes):
                    ...
                    sample = torch.cat([sample[:, :index], img, sample[:, index+2:]], dim=1)
            else:
                img_tokens = img_tokens.view(1, 1, -1)
                ind_insert = ind_insert.nonzero()[0]
                sample = torch.cat([sample[:, :ind_insert], img_tokens, sample[:, ind_insert+2:]], dim=1)                
            new_x.append(sample)
    ...
    def encode_text_img_retrieval(self, text, img_tokens, split_ind=4, repeat=True):
        # text.shape = [1, n_ctx]
        # img_tokens.shape = [batch_size, 2, d_model]        
        #if isinstance(img_tokens, tuple):
        ...
        #        img1 = img_tokens[i][1].view(b_size, 1, -1)
        #        img2 = img_tokens[i][0].view(b_size, 1, -1)
        #        #x = torch.cat([x[:, :index], img, x[:, index+1:]], dim=1)
        #        x = torch.cat([x[:, :index], img1, img2, x[:, index+2:]], dim=1)
        #else:
        #img_tokens = img_tokens.view(b_size, 1, -1)
        img_token1 = img_tokens[0].view(b_size, 1, -1)
        img_token2 = img_tokens[1].view(b_size, 1, -1)
        ...
        x = torch.cat([x[:, :ind_insert], img_token1, 
                        x[:,ind_insert+1:ind_insert+2], img_token2, x[:, ind_insert+3:]], dim=1)
