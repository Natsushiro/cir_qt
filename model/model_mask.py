...
class IM2TEXT_masknet(nn.Module):
    def __init__(self, clip_model, embed_dim=512, middle_dim=512, output_dim=512, n_layer=2, dropout=0.1):
        super().__init__()
        self.mask_token = nn.Parameter(torch.zeros(1, 1, 1024))
        self.proj = deepcopy(clip_model.visual.proj)
        self.fc_out = nn.Linear(middle_dim, output_dim)
        layers = []
        dim = embed_dim
        for _ in range(n_layer):
            ...
            layers.append(nn.Sequential(*block))        
        self.layers = nn.Sequential(*layers)
        self.ln = deepcopy(clip_model.visual.ln_post)
...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention_return(self, x: torch.Tensor):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
    ...
    def forward_attention(self, x: torch.Tensor):
        attention_res = self.attention_return(self.ln_1(x))
...
class Transformer(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        for block in self.resblocks:
            x, weight = block.forward_attention(x)
...
class VisualTransformer(nn.Module):
    ...
    def random_masking(self, x, mask_ratio=0.3):
        N, L, D = x.shape
    ...
    def block_masking(self, x, mask_ratio=0.3):
        N, L ,D = x.shape
        len_keep = int(L * (1 - mask_ratio))
        block_start = torch.randint(0, len_keep, (N,))# N * 1
        ids_all = block_start.view(-1, 1) + torch.arange(L-len_keep)
        new_values = torch.zeros(L-len_keep, D).to(x.dtype).cuda()
        x[torch.arange(N).unsqueeze(1), ids_all] = new_values
        return x
    
    def attention_masking(self, x, mask_ratio, mask_attn):
        N, L ,D = x.shape
        len_keep = int(L * (1 - mask_ratio))
        # mask_attn.shape = N, D or N, 1, D (remains unverified)]
        #ids_mask = torch.argsort(mask_attn, dim=1, descending=True)[:, :L-len_keep]
        ids_mask = torch.argsort(mask_attn, dim=1)[:, :L-len_keep]
        new_values = torch.rand(L-len_keep, D).to(x.dtype).cuda()
        x[torch.arange(N).unsqueeze(1), ids_mask] = new_values
        return x
    ...
    def forward_attn(self, x: torch.Tensor):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        x, weight = self.transformer.attention(x)
        ...
        if self.proj is not None:
            x = x @ self.proj
        last_weight = last_weight[:, 0, 1:]
        last_weight = last_weight.squeeze(1)
        #print(last_weight)
        #print(last_weight.shape)

        return x, last_weight

    #def forward_mask(self, x: torch.Tensor, mask_ratio: float = 0.3):
    #    x = self.conv1(x)  # shape = [*, width, grid, grid]
    #    x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
    #    x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width], being the original
    #    x = x + self.positional_embedding.to(x.dtype)[1:, :]
    #    x, mask, ids_restore = self.random_masking(x)
    #    # after the beforehead line, we got the masked x, with positional embedding already added.
    #    cls_token = self.class_embedding.to(x.dtype) + self.positional_embedding.to(x.dtype)[:1, :]
    ...
    def forward_mask(self, x: torch.Tensor, mask_ratio: float, attn: torch.Tensor = None):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        if attn is not None:
            x = self.attention_masking(x, mask_ratio, attn)
        else:
            x = self.block_masking(x, mask_ratio)
...
class CLIP(nn.Module):
    ...
    def encode_masked_image(self, image, mask_ratio, mask_attn):
        return self.visual.forward_mask(image.type(self.dtype), mask_ratio, mask_attn)

    def encode_image_attn(self, image):
        return self.visual.forward_attn(image.type(self.dtype))
    ...
    def encode_text_img_original(self, text, img_tokens):
    ...
    def encode_text_img(self, text, img_tokens, captions):
        b_size = img_tokens.size(0)
        x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
        cap = self.token_embedding(captions) # [batch_size, n_ctx, d_model]
        collect_ind = text == self.end_id 
        collect_ind = collect_ind.nonzero()[:, 1]
        img_tokens = img_tokens.view(b_size, 1, -1)
        comma_token = self.token_embedding(torch.tensor(267).cuda()) # value of ","
        comma_token = comma_token.repeat(b_size, 1).view(b_size, 1, -1)
        x = torch.cat([x[:, :collect_ind[0]], img_tokens, comma_token, cap[:, 1:]], dim=1)[:, :77]
        # "A photo of * , [cap]"
        x = torch.cat([x[:, :collect_ind[0]], img_tokens, x[:, collect_ind[0]:-1]], dim=1)
        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
